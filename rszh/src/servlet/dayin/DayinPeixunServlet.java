...
public class DayinPeixunServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		for(int i=0;i<Valueslist.size();i++){
			peixunNameList.add((String)Valueslist.get(i).getValues().get("培训名称"));
			peixunTimeList.add((String)Valueslist.get(i).getValues().get("培训时间"));
		}
...
;						table.addCell(par);
		...
		String filePath=request.getSession().getServletContext().getRealPath("/images/2.jpg");
		...
		for(int i=0;i<3;i++){
			...
			//for(int j=0;j<peixunNameList.size();i++){
				if(peixunNameList.get(0)==null){
					 p2= new Paragraph(" ", cn);
				}
				else {
					p2 = new Paragraph(peixunNameList.get(0), cn);
				}
			//}
			Paragraph p3 =new Paragraph();
			//for(int j=0;j<peixunTimeList.size();i++){
				if(peixunTimeList.get(0)==null){
					p3 = new Paragraph(" ", cn);
				}
				else {
					p3 = new Paragraph(peixunTimeList.get(0), cn);
				}
			//}
